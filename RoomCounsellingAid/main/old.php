    ...
    function acceptMessages() {
        if ($_COOKIE['member_name']) 
        {
            if(isset($_POST['s_say']) && $_POST['s_message']) 
            {
                ...
                if ($sMessage != '')
                {
                    mysqli_query($vLink,"INSERT INTO `s_chat_messages` SET `user`='{$sUsername}', `message`='{$sMessages}', `whenat`=UNIX_TIMESTAMP()");
                    mysqli_query($vLink,"INSERT INTO `s_chat_message` SET `user`='{$sUsername}', `message`='{$enc}', `whenat`=UNIX_TIMESTAMP()");
