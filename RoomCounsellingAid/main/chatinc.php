...
class SimpleChat {
    ...
    function getMessages() {
       ...
       // mysql_select_db($this->sDbName);
        ...
        $vRes = mysqli_query($vLink,"SELECT * FROM `dec_messages` ORDER BY `id` ASC LIMIT 15 ");
        ...
        if ($vRes) {
            while($aMessages = mysqli_fetch_array($vRes)) {
                ...
                $sMessages .= '<div class="message">' . $aMessages['user'] . ': ' . $aMessages['demessage'] . '<span>(' . $sWhen . ')</span></div>';
            }
        } else {
